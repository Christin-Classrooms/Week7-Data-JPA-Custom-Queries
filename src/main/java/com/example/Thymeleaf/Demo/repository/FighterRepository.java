...
public interface FighterRepository extends JpaRepository<Fighter, Integer> {
    ...
    // ── Update Query (avoids needing Lombok getters/setters in controller) ─────

    @Modifying
    @Transactional
    @Query("UPDATE Fighter f SET f.name = ?2, f.health = ?3, f.damage = ?4, f.resistance = ?5 WHERE f.id = ?1")
    void updateFighter(int id, String name, Integer health, Double damage, Double resistance);
}